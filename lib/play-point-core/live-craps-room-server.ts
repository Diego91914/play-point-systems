...
function commandFromAction(playerId:string,action:string,payload:Record<string,unknown>):LiveCrapsRoomCommand{
 if(action==="start")return{type:"start",actorPlayerId:playerId};if(action==="tick")return{type:"tick",nowMs:typeof payload.nowMs==="number"?payload.nowMs:undefined};if(action==="begin-roll")return{type:"begin-roll",actorPlayerId:playerId};if(action==="settle-physical")return{type:"settle-physical",actorPlayerId:playerId,die1:Number(payload.die1),die2:Number(payload.die2)};if(action==="settle-virtual")return{type:"settle-virtual",actorPlayerId:playerId};if(action==="close")return{type:"close",actorPlayerId:playerId};throw new Error("Unsupported Live Craps action.");
}
