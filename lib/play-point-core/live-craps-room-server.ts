...
function issueToken(code:string,playerId:string){const key=roomPlayerKey(code,playerId);if(playerTokens.has(key))throw new Error("Live Craps player identity is already in use. Rejoin with the existing session token.");const token=randomUUID();playerTokens.set(key,token);return token;}
...
export function joinLiveCrapsServerRoom(code:string,input:{playerId:string;name:string}){getStoredLiveCrapsRoom(code);const token=issueToken(code,input.playerId);try{applyLiveCrapsRoomCommand(code,`join-${input.playerId}`,{type:"join",playerId:input.playerId,name:input.name});}catch(error){playerTokens.delete(roomPlayerKey(code,input.playerId));throw error;}return{code:code.trim().toUpperCase(),playerId:input.playerId,token,state:projectStoredLiveCrapsRoom(code,input.playerId)};}
