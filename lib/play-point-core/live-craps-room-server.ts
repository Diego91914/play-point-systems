...
function commandFromAction(playerId:string,action:string,payload:Record<string,unknown>):LiveCrapsRoomCommand{
 ...
 if(action==="undo-bet"){if(typeof payload.betId!=="string"||!payload.betId)throw new Error("betId is required.");return{type:"undo-bet",actorPlayerId:playerId,betId:payload.betId};}
