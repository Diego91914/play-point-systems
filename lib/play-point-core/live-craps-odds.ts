...
export type LiveCrapsOddsSettlement = { betId: string; playerId: string; parentBetId: string; status: "won" | "lost" | "working" | "off"; stake: number; credit: number; profit: number };
...
function expectedSide(parentKind: LiveCrapsOddsParentKind): LiveCrapsOddsSide {
  return parentKind === "pass-line" || parentKind === "come" ? "pass" : "dont-pass";
}
...
export function placeLiveCrapsOdds(input: { odds: LiveCrapsOddsBet[]; bankrolls: LiveCrapsBankroll[]; id: string; playerId: string; parentBetId: string; parentKind: LiveCrapsOddsParentKind; side: LiveCrapsOddsSide; point: LiveCrapsPoint; lineAmount: number; amount: number; workingOverride?: LiveCrapsOddsWorkingOverride }) {
  ...
  return {
    odds: [...input.odds, { id: input.id, playerId: input.playerId, parentBetId: input.parentBetId, parentKind: input.parentKind, side: input.side, point: input.point, amount: input.amount, workingOverride: input.workingOverride ?? "table-default" }],
    bankrolls: input.bankrolls.map((item) => item.playerId === input.playerId ? { ...item, chips: item.chips - input.amount } : item),
  };
}

export function settleLiveCrapsOdds(input: { odds: LiveCrapsOddsBet[]; bankrolls: LiveCrapsBankroll[]; total: number; tablePointBefore: LiveCrapsPoint | null }) {
