...
export const LIVE_CRAPS_BET_ZONE_IDS = [
  ...
  "ats-small", "ats-tall", "ats-all", "inside", "across",
] as const;
export type LiveCrapsBetZoneId = typeof LIVE_CRAPS_BET_ZONE_IDS[number];
...
export type LiveCrapsBettingAction = { id: string; playerId: string; seat: number; zoneId: LiveCrapsBetZoneId; amount: number; createdAt: string };
export type LiveCrapsBettingWindow = { open: boolean; selectedAmountByPlayer: Record<string, number>; newActions: LiveCrapsBettingAction[] };
...
export function touchLiveCrapsBetZone(window: LiveCrapsBettingWindow, input: { id: string; playerId: string; seat: number; zoneId: LiveCrapsBetZoneId; createdAt?: string }): LiveCrapsBettingWindow {
  ...
  if (!(LIVE_CRAPS_BET_ZONE_IDS as readonly string[]).includes(input.zoneId)) throw new Error("Unknown Live Craps betting zone.");
  ...
  return { ...window, newActions: [...window.newActions, { ...input, amount, createdAt: input.createdAt ?? new Date().toISOString() }] };
}
...
export function aggregateLiveCrapsNewBets(window: LiveCrapsBettingWindow, playerId: string) {
  ...
  for (const action of window.newActions) if (action.playerId === playerId) totals.set(action.zoneId, (totals.get(action.zoneId) ?? 0) + action.amount);
