...
export type LiveCrapsPlayerRollResult = { playerId: string; bankrollBefore: number; bankrollAfter: number; netDelta: number; atsCredit: number };
export type LiveCrapsPublicPayout = { playerId: string; playerName: string; seat: number; payout: number; netDelta: number; result: "won" | "lost" | "push-or-working" };
export type LiveCrapsSettlementState = { table: LiveCrapsTable; bets: LiveCrapsBet[]; bankrolls: LiveCrapsBankroll[]; ats: LiveCrapsAtsState };
...
export function settleConfirmedLiveCrapsPhysicalRoll(state: LiveCrapsSettlementState, entry: LiveCrapsDiceEntry): LiveCrapsSettlementResult {
  ...
  const playerResults = bankrolls.map((bankroll) => {
    ...
    return { playerId: bankroll.playerId, bankrollBefore, bankrollAfter: bankroll.chips, netDelta: bankroll.chips - bankrollBefore, atsCredit: atsCredits.get(bankroll.playerId) ?? 0 };
  });

  const publicPayouts = table.players
    .map((player) => {
      const result = playerResults.find((item) => item.playerId === player.id)!;
      // Payout is chips returned/credited on this roll; netDelta also reflects losing stakes already removed when placed.
      const payout = Math.max(0, result.bankrollAfter - result.bankrollBefore);
      return { playerId: player.id, playerName: player.name, seat: player.seat, payout, netDelta: result.netDelta, result: result.netDelta > 0 ? "won" as const : result.netDelta < 0 ? "lost" as const : "push-or-working" as const };
    })
    .sort((a, b) => a.seat - b.seat);

  return { table, bets: regular.bets, bankrolls, ats, roll: { id: history.id, shooterId: history.shooterId, die1: history.die1, die2: history.die2, total: history.total }, playerResults, publicPayouts };
}
...
export function projectLiveCrapsSettlementForPlayer(result: LiveCrapsSettlementResult, playerId: string) {
  ...
  return { roll: result.roll, point: result.table.point, shooterSeat: result.table.shooterSeat, atsCovered: [...result.ats.covered], publicPayouts: result.publicPayouts, personal, personalBets: result.bets.filter((bet) => bet.playerId === playerId) };
}
