...
function requiredGameSku(pathname: string): string | null {
  if (
    pathname === "/games/holdem" ||
    pathname.startsWith("/games/holdem/") ||
    pathname === "/api/games/holdem" ||
    pathname.startsWith("/api/games/holdem/")
  ) {
    return "game.phone_holdem";
  }

  if (
    pathname === "/games/trivia" ||
    pathname.startsWith("/games/trivia/") ||
    pathname === "/api/trivia" ||
    pathname.startsWith("/api/trivia/")
  ) {
...
export async function proxy(request: NextRequest) {
  ...
  if (gameSku && !gamesSessionOwns(claims, gameSku)) {
    if (isApiRequest(pathname)) {
      return NextResponse.json(
        { error: "This game is not owned by the signed-in account." },
        { status: 403, headers: { "Cache-Control": "private, no-store" } }
      );
    }

    const target = request.nextUrl.clone();
    target.pathname = "/games";
    target.search = "";
    target.searchParams.set("locked", gameSku);
    return NextResponse.redirect(target);
  }
