...
export const metadata: Metadata = {
  title: "Play Point Live Football MVP Player Lobby",
  description:
    "Player-facing Play Point Live football MVP lobby for viewing picks, triggers, and live standings.",
};
        ...
        <div className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-end xl:gap-14">
          <div className="max-w-4xl reveal-up">
            ...
            <h1 className="mt-6 max-w-4xl text-5xl font-black tracking-tight text-white sm:text-6xl lg:text-7xl xl:text-[5rem] xl:leading-[0.96]">
              Follow the football MVP from the player side.
            </h1>
            <p className="mt-5 max-w-3xl text-lg font-semibold leading-8 text-cyan-100/88 sm:text-xl sm:leading-8">
              Live standings, posted triggers, and your contest cards all read from the
              same scoring kernel as the host dashboard.
            </p>
          ...
          <div className="rounded-[30px] border border-white/10 bg-[linear-gradient(180deg,rgba(111,182,255,0.12),rgba(255,255,255,0.03))] p-6 shadow-[0_24px_70px_rgba(0,0,0,0.2)]">
            ...
            <ul className="mt-5 grid gap-3 text-sm text-white/78">
              <li className="flex items-start gap-3 rounded-2xl border border-white/8 bg-black/20 px-4 py-3">
                ...
                <span>Shows the seeded player entries already attached to the football MVP event.</span>
              </li>
              <li className="flex items-start gap-3 rounded-2xl border border-white/8 bg-black/20 px-4 py-3">
                <span className="mt-1 h-2.5 w-2.5 rounded-full bg-amber-300" />
                <span>Tracks only settled state, so the player sees what the engine has actually resolved.</span>
              </li>
              <li className="flex items-start gap-3 rounded-2xl border border-white/8 bg-black/20 px-4 py-3">
                <span className="mt-1 h-2.5 w-2.5 rounded-full bg-amber-300" />
                <span>Updates safely after corrections because superseded resolution rows fall out of the active view.</span>
              </li>
            </ul>
          </div>
        </div>
