...
export async function GET(request: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, ...await getAllAboutYouRoom(code, playerId, token) });
  } catch (error) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, ...await actAllAboutYouRoom(code, body.playerId, body.token, body.action, body.payload) });
  } catch (error) {
