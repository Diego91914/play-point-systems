...
type Occasion = "birthday" | "celebration" | "just-because";
type Game = {
  code: string;
...
const OCCASIONS: { value: Occasion; label: string; note: string }[] = [
  { value: "birthday", label: "Birthday", note: "Birthday-first framing with a couple of celebration-flavored rounds." },
  { value: "celebration", label: "Celebration", note: "Great for retirement, graduation, promotion, anniversary, or a big milestone." },
  { value: "just-because", label: "Just Because", note: "Pure 'how well do you know them?' with no event-specific prompt bias." },
];
...
export function AllAboutYouClient() {
  ...
  return <main className="px-4 py-7 sm:px-8"><div className="mx-auto max-w-3xl">
    ...
    {game.status === "lobby" && <section className="mt-6 rounded-[28px] border border-white/10 bg-white/[.04] p-6"><h2 className="text-2xl font-black text-white">Set the occasion</h2><p className="mt-2 text-sm leading-6 text-white/60">Birthday is the default, but the same game can celebrate a milestone or simply put somebody at center stage for no special reason.</p><div className="mt-4 grid gap-2 sm:grid-cols-3">{OCCASIONS.map(option => <button key={option.value} disabled={busy || !game.me?.isHost} onClick={() => act("set-occasion", { occasion: option.value })} className={`rounded-2xl border p-4 text-left ${game.occasion === option.value ? "border-fuchsia-300/50 bg-fuchsia-300/15" : "border-white/10 bg-black/20"}`}><div className="font-black text-white">{game.occasion === option.value ? "✓ " : ""}{option.label}</div><div className="mt-1 text-xs leading-5 text-white/45">{option.note}</div></button>)}</div><div className="mt-6 border-t border-white/10 pt-6"><h2 className="text-2xl font-black text-white">Choose the Guest of Honor</h2><p className="mt-2 text-sm leading-6 text-white/60">The star stays at center stage for all five rounds. Everyone else competes to prove who knows them best.</p><RoomJoinPanel code={game.code} joinUrl={joinUrl} gameName="All About You"/><div className="mt-5 grid grid-cols-2 gap-2">{game.players.map(player => <button key={player.id} disabled={busy || !game.me?.isHost} onClick={() => act("set-guest", { guestId: player.id })} className={`rounded-2xl border px-3 py-3 text-sm font-black ${player.id === game.guestId ? "border-fuchsia-300/50 bg-fuchsia-300/15 text-fuchsia-50" : "border-white/10 bg-black/20 text-white/70"}`}>{player.id === game.guestId ? "⭐ " : ""}{player.name}</button>)}</div>{game.me?.isHost ? <button disabled={busy || game.players.length < 3 || !game.guestId} onClick={() => act("start")} className="mt-5 w-full rounded-2xl bg-fuchsia-300 px-4 py-4 font-black text-slate-950 disabled:opacity-40">START · 5 ROUNDS</button> : <p className="mt-5 text-center text-sm text-white/50">Waiting for the host…</p>}<p className="mt-3 text-center text-xs text-white/35">At least 3 people total.</p></div></section>}
