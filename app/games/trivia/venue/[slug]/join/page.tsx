...
export default function TriviaVenueJoinPage() {
  ...
  async function join() {
    ...
    try {
      ...
      const next: StoredVenuePlayer = { playerId: payload.player.id, deviceToken: payload.deviceToken, venueSessionId: payload.venueSessionId, name: payload.player.name, presenceExpiresAt: payload.player.presence_expires_at };
      window.localStorage.setItem(storageKey, JSON.stringify(next));
      setStoredPlayer(next);
    ...
    <GameAtmosphere variant="trivia">
      <main className="mx-auto min-h-screen w-full max-w-xl px-5 py-10 text-white">
        <div className="rounded-[32px] border border-white/10 bg-black/30 p-6 shadow-2xl backdrop-blur sm:p-8">
          ...
          <p className="mt-3 text-sm leading-7 text-white/68">{storedPlayer ? `You're saved as ${storedPlayer.name}. One scan keeps your same name and score for another hour.` : "Enter a nickname. Your phone becomes your private answer pad while the TV runs the show."}</p>
          {!hasCurrentQr ? <div className="mt-6 rounded-2xl border border-amber-300/25 bg-amber-300/10 p-4 text-sm font-semibold text-amber-100">Scan the current QR code on the venue TV to {storedPlayer ? "keep playing" : "join"}.</div> : null}
          {storedPlayer ? (
            <button type="button" disabled={!hasCurrentQr || joining} onClick={continuePlaying} className="mt-7 w-full rounded-2xl bg-cyan-300 px-5 py-4 text-base font-black text-[#05131d] disabled:opacity-40">{joining ? "Renewing…" : "KEEP MY PLACE"}</button>
          ) : (
            <>
