...
export function PlayingCard({ card, hidden = false, small = false, delay = 0, reveal = false }: { card?: Card; hidden?: boolean; small?: boolean; delay?: number; reveal?: boolean }) {
  const size = small ? "h-[64px] w-[44px] text-base sm:h-[72px] sm:w-[50px]" : "h-[86px] w-[58px] text-lg sm:h-[108px] sm:w-[72px] sm:text-xl";
  if (hidden || !card) return <div className={`${size} holdem-card-back holdem-card-deal shrink-0 rounded-xl border border-cyan-100/30 shadow-[0_10px_28px_rgba(0,0,0,0.4)]`} style={{ animationDelay: `${delay}ms` }} aria-label="Hidden playing card" />;
  const { rank, suit, red } = cardParts(card);
  return <div className={`${size} ${reveal ? "holdem-board-reveal" : "holdem-card-deal"} flex shrink-0 flex-col justify-between rounded-xl border border-white/90 bg-[linear-gradient(145deg,#fff,#edf1f4)] p-2 font-black shadow-[0_12px_30px_rgba(0,0,0,0.42)] ${red ? "text-red-600" : "text-slate-950"}`} style={{ animationDelay: `${delay}ms` }} aria-label={`${rank}${suit}`}><span>{rank}</span><span className="self-end text-2xl leading-none">{suit}</span></div>;
}
...
function orderedAroundViewer(players: SurfacePlayer[], viewerSeat: number | null) {
  ...
  return [...players].sort((a, b) => ((a.seat - viewerSeat + 8) % 8) - ((b.seat - viewerSeat + 8) % 8));
}
...
function seatPosition(index: number, total: number) {
  ...
  return { left: `${50 + Math.cos(angle) * horizontalRadius}%`, top: `${50 + Math.sin(angle) * verticalRadius}%` };
}

function PlayerSeat({ player, position, isMe, isWinner = false, isResult = false }: { player: SurfacePlayer; position: { left: string; top: string }; isMe: boolean; isWinner?: boolean; isResult?: boolean }) {
  ...
  return (
    <div className={`absolute w-[112px] -translate-x-1/2 -translate-y-1/2 rounded-2xl border px-2.5 py-2 text-center shadow-[0_14px_32px_rgba(0,0,0,0.38)] transition duration-300 sm:w-[136px] sm:px-3 sm:py-2.5 ${isWinner ? "z-50 holdem-winning-seat border-amber-100 bg-[#2b2108]/[.99] shadow-[0_0_0_4px_rgba(252,211,77,.24),0_0_58px_rgba(252,211,77,.48)]" : "z-20"} ${!isWinner && player.isTurn ? "holdem-turn-pulse border-amber-300/80 bg-[#221b08]" : !isWinner && isMe ? "border-cyan-300/45 bg-[#071a21]/95" : !isWinner ? "border-white/15 bg-black/80" : ""} ${folded || player.sittingOut ? "opacity-35 grayscale" : resultDimmed ? "opacity-40" : "opacity-100"}`} style={position}>
      {isWinner && <div className="absolute -top-4 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border-2 border-white/65 bg-amber-300 px-3 py-1 text-[9px] font-black uppercase tracking-[0.18em] text-amber-950 shadow-xl sm:text-[10px]">Winner</div>}
      ...
      {(player.holeCards.length > 0 || showHiddenCards) && <div className="mt-1.5 flex justify-center -space-x-2">{player.holeCards.length > 0 ? player.holeCards.map((card, index) => <PlayingCard key={card} card={card} small delay={index * 120} />) : [0, 1].map((index) => <PlayingCard key={`hidden-${player.id}-${index}`} hidden small delay={index * 120} />)}</div>}
    </div>
  );
}

export function HoldemTableSurface({ players, board, pot, street, handNumber, winners, viewerId, publicMode = false }: { players: SurfacePlayer[]; board: Card[]; pot: number; street: string; handNumber: number; winners: SurfaceWinner[]; viewerId?: string | null; publicMode?: boolean }) {
  ...
  return (
    <div className={`holdem-table-surface relative min-h-[440px] overflow-hidden rounded-[34px] border bg-[#04120e] shadow-[0_28px_80px_rgba(0,0,0,0.42)] sm:min-h-[600px] sm:rounded-[42px] ${isResult ? "border-amber-300/40" : "border-emerald-200/20"}`}>
      <style>{`
        ...
        @keyframes holdemWinningSeat { 0% { transform:translate(-50%,-50%) translateY(12px) scale(.94); opacity:.45; } 45% { transform:translate(-50%,-50%) translateY(-12px) scale(1.12); opacity:1; } 100% { transform:translate(-50%,-50%) translateY(-8px) scale(1.08); opacity:1; } }
      ...
      <div className={`absolute left-1/2 top-1/2 w-[82%] -translate-x-1/2 -translate-y-1/2 text-center sm:w-auto ${isResult ? "z-40" : "z-10"}`}>
        ...
        <div className={`mt-3 flex origin-center justify-center gap-1.5 sm:gap-2 ${isResult ? "scale-[0.68] opacity-70 sm:scale-[0.82]" : "scale-[0.78] sm:scale-100"}`}>{Array.from({ length: 5 }, (_, index) => { const card = board[index]; return card ? <PlayingCard key={`${index}-${card}`} card={card} reveal delay={index < 3 ? index * 100 : 0} /> : <div key={`empty-${index}`} className="h-[86px] w-[58px] rounded-xl border border-dashed border-white/12 bg-black/10 sm:h-[108px] sm:w-[72px]" />; })}</div>
        {isResult && <div className="holdem-winner-pop relative z-40 mx-auto -mt-1 w-full max-w-lg rounded-[24px] border-2 border-amber-200/80 bg-[linear-gradient(145deg,rgba(35,25,3,.99),rgba(0,0,0,.98))] px-4 py-4 shadow-[0_0_72px_rgba(252,211,77,.42)] sm:mt-2 sm:px-6 sm:py-5">
          ...
          <div className="mt-2 grid gap-4">{winners.map((winner) => <div key={`winner-reveal-${winner.playerId}`}>
            <div className="text-3xl font-black leading-none text-white sm:text-5xl">{winner.name}</div>
            {winner.handName === "Uncontested" ? <div className="mt-2 text-base font-black text-amber-200 sm:text-xl">Wins uncontested</div> : <><div className="mt-2 text-sm font-black text-white/70 sm:text-lg">Wins with <span className="text-amber-200">{winner.handName}</span></div>{winner.bestFive?.length > 0 && <div className="mt-3 flex origin-center scale-[0.82] justify-center -space-x-1 sm:scale-100">{winner.bestFive.map((card, index) => <PlayingCard key={`winner-hand-${winner.playerId}-${card}-${index}`} card={card} small reveal delay={index * 70} />)}</div>}</>}
            <div className="mt-2 text-sm font-black text-emerald-200">+{formatChips(winner.amount)} chips</div>
          </div>)}</div>
        </div>}
      </div>
      {ordered.map((player, index) => <PlayerSeat key={player.id} player={player} position={seatPosition(index, ordered.length)} isMe={player.id === viewerId} isWinner={winningIds.has(player.id)} isResult={isResult} />)}
    </div>
  );
}
