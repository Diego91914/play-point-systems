...
describe("Live Craps Buy/Lay", () => {
  ...
  it("charges the standard 5 percent commission only on a win", () => {
    ...
    const won = settleLiveCrapsBuyLay({ ...placed, total: 4 });
  ...
  it("Lay wins on seven before its number", () => {
    ...
    const won = settleLiveCrapsBuyLay({ ...placed, total: 7 });
  ...
  it("Buy loses on seven and Lay loses when its number rolls", () => {
    ...
    expect(settleLiveCrapsBuyLay({ ...buy, total: 7 }).settlements[0].status).toBe("lost");
    const lay = placeLiveCrapsBuyLayBet({ bets: [], bankrolls, id: "l", playerId: "a", kind: "lay", number: 10, amount: 40 });
    expect(settleLiveCrapsBuyLay({ ...lay, total: 10 }).settlements[0].status).toBe("lost");
  });
  it("can mark Buy/Lay wagers off without resolving them", () => {
    ...
    const off = settleLiveCrapsBuyLay({ ...placed, total: 7, working: false });
    expect(off.bets).toHaveLength(1);
    expect(off.settlements[0].status).toBe("working");
  });
});
