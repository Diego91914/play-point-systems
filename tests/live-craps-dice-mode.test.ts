...
describe("Live Craps dice modes", () => {
  test("allows a mode change only with point off, no pending roll, and betting unlocked", () => {
    ...
    expect(
      canChangeLiveCrapsDiceMode({ ...open, point: 6 as const }),
    ).toBe(false);
    expect(
      canChangeLiveCrapsDiceMode({ ...open, pendingRollId: "roll-1" }),
    ).toBe(false);
    expect(
      canChangeLiveCrapsDiceMode({ ...open, bettingLocked: true }),
    ).toBe(false);
  });
  ...
  test("commits both die faces and total before reveal", () => {
    const roll = commitVirtualLiveCrapsRoll({
      rollId: "roll-1",
      shooterId: "p1",
  ...
  test("is replay-safe for an already committed roll id", () => {
    const original = commitVirtualLiveCrapsRoll({
      rollId: "roll-1",
      shooterId: "p1",
    });

    const replay = commitVirtualLiveCrapsRoll({
      rollId: "roll-1",
      shooterId: "p1",
      existing: original,
    });
  ...
  test("reveal projection exposes the authoritative committed result", () => {
    ...
    expect(projectCommittedRollForReveal(roll)).toEqual({
      rollId: roll.rollId,
